...
describe("FS", () => {
  ...
  describe("tempDirectory()", () => {
    ...
    it("creates a temporary directory with given name", async () => {
      const dir = await FS.tempDirectory("test-temp")
      tempDirs.push(dir)

      assert.ok(dir instanceof (await import("../../src/lib/DirectoryObject.js")).default)
      assert.ok(await dir.exists)
      assert.ok(dir.temporary)
      assert.ok(dir.name.startsWith("test-temp-"))
    })

    it("creates directory in OS temp directory", async () => {
      const {default: os} = await import("node:os")
      const dir = await FS.tempDirectory("test-os-temp")
      tempDirs.push(dir)

      const osTempDir = os.tmpdir()
      assert.ok(dir.path.startsWith(osTempDir))
    })

    it("creates unique directories for same name", async () => {
      const dir1 = await FS.tempDirectory("test-unique")
      const dir2 = await FS.tempDirectory("test-unique")
      tempDirs.push(dir1, dir2)

      assert.notEqual(dir1.path, dir2.path)
      assert.ok(await dir1.exists)
      assert.ok(await dir2.exists)
    })

    it("appends hyphen to name if not present", async () => {
      const dir = await FS.tempDirectory("nohyphen")
      tempDirs.push(dir)

      assert.ok(dir.name.startsWith("nohyphen-"))
    })

    it("does not double-append hyphen", async () => {
      const dir = await FS.tempDirectory("withhyphen-")
      tempDirs.push(dir)

      assert.ok(dir.name.startsWith("withhyphen-"))
      assert.ok(!dir.name.includes("--"))
    })

    it("creates subdirectory within temporary parent", async () => {
      const parent = await FS.tempDirectory("parent")
      tempDirs.push(parent)

      const child = await FS.tempDirectory("child", parent)

      assert.ok(await child.exists)
      assert.ok(child.temporary)
      assert.ok(child.path.startsWith(parent.path))
    })

    it("validates parent must be temporary", async () => {
      const regularDir = new (await import("../../src/lib/DirectoryObject.js")).default(process.cwd())

      await assert.rejects(
        () => FS.tempDirectory("child", regularDir),
        /Parent must be a temporary DirectoryObject/
      )
    })

    it("validates parent must exist", async () => {
      const parent = await FS.tempDirectory("parent")
      tempDirs.push(parent)
      await parent.delete()

      await assert.rejects(
        () => FS.tempDirectory("child", parent),
        /Parent must exist/
      )
    })

    it("validates parent lineage is in OS temp directory", async () => {
      const {default: DirectoryObject} = await import("../../src/lib/DirectoryObject.js")
      const testPath = await TestUtils.createTestDir("fake-temp")
      const fakeParent = new DirectoryObject(testPath, true)

      try {
        await assert.rejects(
          () => FS.tempDirectory("child", fakeParent),
          /lineage of this directory must be the OS temp directory/
        )
      } finally {
        await TestUtils.cleanupTestDir(testPath)
      }
    })

    it("validates name parameter type", async () => {
      await assert.rejects(
        () => FS.tempDirectory(123),
        /Invalid type/
      )

      await assert.rejects(
        () => FS.tempDirectory(null),
        /Invalid type/
      )
    })

    it("validates parent parameter type", async () => {
      await assert.rejects(
        () => FS.tempDirectory("test", "not-a-directory-object"),
        /Invalid type/
      )
    })

    it("rejects path traversal attacks with parent", async () => {
      const parent = await FS.tempDirectory("parent")
      tempDirs.push(parent)

      await assert.rejects(
        () => FS.tempDirectory("../../../etc", parent),
        /must not contain path separators/
      )
    })

    it("rejects absolute paths with parent", async () => {
      const parent = await FS.tempDirectory("parent")
      tempDirs.push(parent)

      await assert.rejects(
        () => FS.tempDirectory("/etc/passwd", parent),
    ...
    it("rejects names with path separators", async () => {
      ...
      await assert.rejects(
        () => FS.tempDirectory("foo/bar", parent),
        /must not contain path separators/
      )
    ...
    it("rejects empty name with parent", async () => {
      const parent = await FS.tempDirectory("parent")
      tempDirs.push(parent)

      await assert.rejects(
        () => FS.tempDirectory("", parent),
        /must not be empty/
      )
    })
  })
